...
??? proof "证明"
    ...
    从任意一个点 $u$ 开始。若 $u$ 已满足删去后所有连通块大小均不超过 $\frac n2$，那么 $u$ 就是重心。

    否则，必然存在一个连通块大小大于 $\frac n2$。向这个连通块所在方向移动到相邻节点 $v$。

    可以发现，移动之后，原来那个过大的连通块大小会变小，而其他部分合并到 $v$ 的父亲方向中。由于每次都向唯一的大连通块移动，所以这个过程不可能在树上无限进行。

    最终一定会停在一个点，使得删去它后所有连通块大小均不超过 $\frac n2$，即为重心。
...
??? proof "证明"
    ...
    因为 $u$ 是重心，删去 $u$ 后，包含 $v$ 的那个连通块大小不超过 $\frac n2$。

    若 $v$ 不与 $u$ 相邻，则从 $u$ 向 $v$ 移动时，$u$ 方向剩余的连通块会严格大于 $\frac n2$，从而 $v$ 不可能是重心，矛盾。
