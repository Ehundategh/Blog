...
设区间长度为 $len=r-l+1$，取 $k=\lfloor\log_2 len\rfloor$，那么 $2^k\le len$。我们取两个长度为 $2^k$ 的区间：
...
预处理时一共有 $\mathcal{O}(\log n)$ 层，每一层最多有 $\mathcal{O}(n)$ 个状态，所以预处理复杂度为 $\mathcal{O}(n\log n)$。

每次查询只需要取两个预处理好的区间，复杂度为 $\mathcal{O}(1)$，空间复杂度为 $\mathcal{O}(n\log n)$。
