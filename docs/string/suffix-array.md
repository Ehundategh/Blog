...
考虑倍增。我们先按长度为 $1$ 的前缀排序，也就是按单个字符排序。然后若已经知道所有后缀长度为 $k$ 的前缀排名，就可以尝试推出长度为 $2k$ 的前缀排名。

对于起点为 $i$ 的后缀，长度为 $2k$ 的前缀可以拆成两段：
...
$$
s_{i+k}\cdots s_{i+2k-1}
$$

第一段的排名就是 $\operatorname{Rank}_i$，第二段的排名就是 $\operatorname{Rank}_{i+k}$。因此可以用二元组：
...
![倍增求后缀数组](../assets/string/sa-doubling.svg)
...
若每一轮直接对所有二元组排序，单轮复杂度为 $\mathcal{O}(n\log n)$，总复杂度为 $\mathcal{O}(n\log^2 n)$。

接下来考虑如何把单轮排序优化到 $\mathcal{O}(n)$。

注意到二元组中的两个关键字都是排名，范围为 $[1,n]$，因此可以使用计数排序。并且对于二元组：

$$
(A_i,B_i)
$$

若已经按照第二关键字 $B_i$ 排好，再按第一关键字 $A_i$ 做一次稳定排序，就可以得到完整的二元组排序。

后缀数组倍增中，第二关键字为 $\operatorname{Rank}_{i+k}$。如果某个位置 $x$ 在当前的 $\operatorname{Sa}$ 中出现，那么 $x-k$ 的第二关键字对应的正好是位置 $x$ 的第一段排名。因此我们可以按下面的方式构造 `Second`：

1. 先放入 $n-k+1,\ldots,n$，这些位置的第二关键字为空，排名最小。
2. 再按当前 $\operatorname{Sa}$ 的顺序枚举，如果 $\operatorname{Sa}_i>k$，就放入 $\operatorname{Sa}_i-k$。

这样得到的 `Second` 已经按第二关键字排好。之后只需要按第一关键字 $\operatorname{Rank}$ 做一遍计数排序，就得到新一轮的 $\operatorname{Sa}$。

由于每一轮只做常数次线性扫描和计数排序，所以总复杂度为：
...
这段代码中，`Second` 表示这一轮排序中已经按第二关键字排好的下标顺序。由于第一关键字是 `Rank`，再对 `Second` 按 `Rank` 做一次计数排序，就得到了二元组排序结果。

每轮排序结束后，需要重新编号。若相邻两个后缀的二元组相同，则它们的新排名相同；否则新排名加一。这个新排名会作为下一轮倍增的 $\operatorname{Rank}$。
